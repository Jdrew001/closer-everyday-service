...
namespace CED.Services.Core
{
  public class HabitService : IHabitService
  {
    ...
    private readonly IDashboardGraphContext _dashboardGraphContext;
    ...
    public HabitService(
        ...
        IFriendService friendService,
         IDashboardGraphContext dashboardGraphContext)
    {
      ...
      _log = log;
      _dashboardGraphContext = dashboardGraphContext;
    }
    ...
    public async Task<List<GraphDataResponseDTO>> GetGraphData<T, K>(Guid userId, T request)
    {
      var logs = await _habitRepository.GetAllLogsForUser(userId);

      if (logs.Count == 0)
        return new List<GraphDataResponseDTO> { };

      _dashboardGraphContext.SetStrategy<K>();
      return _dashboardGraphContext.CreateDataForGraph(request, logs);
    }
  }
}
